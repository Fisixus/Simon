...
- **Assets/_Project/Scripts/Core**: Contains the architectural foundation.
    ...
    - **MVC**: Base classes for Views and Controllers, and reactive utilities.
...
- **Model**: Data classes using `ReactiveProperty<T>` to notify controllers/views of state changes.
...
- **Reactive Model**: Use `ReactiveProperty<T>` to subscribe to value changes within the controller.
...
- **Reactivity**: Prefer `ReactiveProperty<T>` for model state and event-driven updates.
